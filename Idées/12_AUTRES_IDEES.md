# Autres idées
> Sources : Notion — fiches individuelles

Projets en germe, sans mécanique définie. Chaque idée mérite sa propre fiche si elle se développe.

---

## Jeux de type "Reign"

| | |
|---|---|
| **Thème** | Prise de décision, Gouvernance |
| **État** | Idée |
| **Type** | Cartes |
| **Joueurs** | 1 à 4 |
| **Difficulté** | Moyen |
| **Interaction** | Coopération |
| **Envie de dév.** | 0 [Source Notion — échelle non définie] |
| **Inspirations** | *Reign* |

> Jeu de cartes à prises de décision inspiré du jeu *Reign*. Aucune mécanique définie au-delà du thème.

---

## Gestion de Flore/Faune pour création forêt

| | |
|---|---|
| **Thème** | Nature, Construction, Écosystème |
| **État** | Idée |
| **Type** | Cartes, Plateau |
| **Joueurs** | 1 à 4 |
| **Difficulté** | Difficile |
| **Interaction** | Coopération |
| **Envie de dév.** | 1 [Source Notion — échelle non définie] |
| **Inspirations** | — |

> Idée de jeu coopératif autour de la construction d'une forêt en gérant flore et faune. Difficulté prévue élevée. Aucune mécanique définie.

---

## Jeu sur les sens

| | |
|---|---|
| **Thème** | Sens (vue, ouïe, toucher, odorat, goût) |
| **État** | Idée |
| **Type** | Ambiance |
| **Joueurs** | 4 exactement |
| **Difficulté** | Simple |
| **Interaction** | Coopération |
| **Envie de dév.** | 0 [Source Notion — échelle non définie] |
| **Inspirations** | — |

> Jeu d'ambiance coopératif à 4 joueurs mettant en scène les 5 sens. Format simple. Aucune mécanique définie.

---

## Jeu abeille — gestion d'équipe en entreprise

| | |
|---|---|
| **Thème** | Abeilles, Entreprise, Travail d'équipe |
| **État** | Idée |
| **Type** | Non défini |
| **Joueurs** | Non défini |
| **Difficulté** | Non définie |
| **Interaction** | Non définie |
| **Envie de dév.** | — [Non renseigné] |
| **Inspirations** | — |

> Concept autour de la colonie d'abeilles comme métaphore de la gestion d'équipe en entreprise. Aucune donnée supplémentaire documentée.


---

<!-- SOURCE-ARCHIVE:BEGIN -->
## Annexe — Notes sources exhaustives

<details>
<summary>Afficher la transcription exhaustive et les variantes historiques</summary>

> Cette annexe est la couche de référence restaurée depuis Mixologie avant restructuration (7db5550). Elle ne doit pas être condensée. Seuls les chemins des médias ont été adaptés à la nouvelle arborescence.

> **Couche exhaustive — Jeu de type Reign.** Ce document conserve les formulations, variantes, répétitions, dates, tableaux et médias issus des exports. Il fait foi lorsqu’une synthèse éditoriale simplifie ou interprète un point.

# Jeux de type “Reign”

## Fiche projet

| Propriété | Valeur |
|---|---|
| Initiateur | Yann Danjaume |
| État | Idée |
| Interaction | Coopération |
| Type de jeu | Cartes |
| Nbr joueurs | 1 à 4 |
| Difficulté | Moyen |
| Envie de développer | 0 |
| Thème | Prise de décision |

Reign

---

> **Couche exhaustive — Gestion de forêt.** Ce document conserve les formulations, variantes, répétitions, dates, tableaux et médias issus des exports. Il fait foi lorsqu’une synthèse éditoriale simplifie ou interprète un point.

# Gestion de Flore/Faune pour création forêt

## Fiche projet

| Propriété | Valeur |
|---|---|
| Initiateur | Yann Danjaume |
| État | Idée |
| Interaction | Coopération |
| Type de jeu | Cartes, Plateau |
| Nbr joueurs | 1 à 4 |
| Difficulté | Difficile |
| Envie de développer | 1 |
| Thème | Construction |

---

> **Couche exhaustive — Jeu sur les sens.** Ce document conserve les formulations, variantes, répétitions, dates, tableaux et médias issus des exports. Il fait foi lorsqu’une synthèse éditoriale simplifie ou interprète un point.

# Jeu sur les sens

## Fiche projet

| Propriété | Valeur |
|---|---|
| Initiateur | Yann Danjaume |
| État | Idée |
| Interaction | Coopération |
| Type de jeu | Ambiance |
| Nbr joueurs | 4 |
| Difficulté | Simple |
| Envie de développer | 0 |
| Thème | Sens |

---

> **Couche exhaustive — Abeilles et gestion d’équipe.** Ce document conserve les formulations, variantes, répétitions, dates, tableaux et médias issus des exports. Il fait foi lorsqu’une synthèse éditoriale simplifie ou interprète un point.

# Jeu abeille gestion d’equipe en entreprise

## Fiche projet

| Propriété | Valeur |
|---|---|
| État | Idée |

</details>
<!-- SOURCE-ARCHIVE:END -->
