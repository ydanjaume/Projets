# Sport — Volley
> Sources : OneNote `Sport - Volley.docx` (13 août 2025) + Notion `Match de volley.md`

---

## Résumé

| | |
|---|---|
| **Thème** | Volley-ball, Sport d'équipe |
| **État** | Idée |
| **Type** | Plateau — versus en équipe |
| **Joueurs** | 2 à 4 |
| **Difficulté** | Moyen |
| **Interaction** | Versus en équipe |
| **Envie de dév.** | 0 [Source Notion — échelle non définie] |
| **Inspirations** | Haikyuu |

> *Idée de jeu de plateau inspiré de l'univers du volley-ball, probablement du manga/anime Haikyuu. Aucune mécanique définie à ce stade.*

---

## Contenu disponible

- Référence citée : **Haikyuu**
- OneNote : section créée le 13 août 2025, aucun contenu documenté
- Notion : fiche avec métadonnées uniquement

*(Concept en attente de développement)*

---

## Questions ouvertes

- [ ] Quelle mécanique de plateau pour simuler le volley ?
- [ ] Gestion individuelle des joueurs ou gestion d'équipe par cartes ?


---

<!-- SOURCE-ARCHIVE:BEGIN -->
## Annexe — Notes sources exhaustives

<details>
<summary>Afficher la transcription exhaustive et les variantes historiques</summary>

> Cette annexe est la couche de référence restaurée depuis Mixologie avant restructuration ($SourceCommit). Elle ne doit pas être condensée. Seuls les chemins des médias ont été adaptés à la nouvelle arborescence.

> **Couche exhaustive — ChatGPT V2.** Ce document conserve les formulations, variantes, répétitions, dates, tableaux et médias issus des exports. Il fait foi lorsqu’une synthèse éditoriale simplifie ou interprète un point.

# Match de volley

## Fiche projet

| Propriété | Valeur |
|---|---|
| Initiateur | Yann Danjaume |
| État | Idée |
| Interaction | Versus en équipe |
| Type de jeu | Plateau |
| Nbr joueurs | 2 à 4 |
| Difficulté | Moyen |
| Envie de développer | 0 |
| Thème | Volley |

Haikyuu

## Notes OneNote

mercredi 13 août 2025

12:10

</details>
<!-- SOURCE-ARCHIVE:END -->
